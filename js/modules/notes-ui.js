...
class NotesUI {
  ...
  _renderNoteCard(note) {
    ...
    const safeColor = _CAT_COLORS.includes(categoryColor) ? categoryColor : '';
        ...
        <div class="note-card-swipeable">
          <div class="note-card" role="button" tabindex="0" aria-label="${escapedTitle}" data-id="${note.id}"${borderStyle}>
            ...
            <div class="note-card-meta">
              ...
              ${note.categoryId ? `<span class="note-card-category">${categoryColor ? `<span class="cat-color-dot" style="background:${categoryColor};"></span>` : ''}${categoryName}</span>` : ''}
  ...
  _renderTagChips(tags) {
    ...
    this._els.tagChips.innerHTML = tags.map(t => `<span class="note-tag-chip">#${t}</span>`).join('');
  }
  ...
  _updateCatLabel() {
    ...
    if (val) {
      ...
      label.innerHTML = color
        ? `<span class="cat-color-dot" style="background:${color};"></span>${window.HTMLEscape(name)}`
