if (!window.UrlValidator) {
  throw new Error('UrlValidator must be loaded before sync-service.js');
}
window.SyncService = class SyncService {
  constructor() {
    const persistedUrl = localStorage.getItem('sync-server-url');
    this.serverUrl = persistedUrl && window.UrlValidator.isAllowedEndpoint(persistedUrl)
      ? window.UrlValidator.normalizeUrl(persistedUrl)
      : 'https://sync.focusedword.com';
  ...
  async revokeSyncKey() {
    const response = await fetch(`${this.serverUrl}/api/sync/${encodeURIComponent(this.syncKey)}`, { method: 'DELETE' });
  ...
  setServerUrl(url) {
    ...
    if (!window.UrlValidator.isAllowedEndpoint(normalized)) {
      throw new Error('Server URL must use HTTPS (or HTTP for localhost development only)');
    }
    this.serverUrl = normalized;
    localStorage.setItem('sync-server-url', normalized);
  }

  setKey(key) {
    const raw = key ? key.trim() : null;
    if (raw && !window.UrlValidator.isValidSyncKey(raw)) {
      console.warn('[SyncService] Invalid sync key format — key must be 8-256 alphanumeric/hyphen characters');
      this.syncKey = null;
      localStorage.removeItem('sync-key');
      return;
    }
    this.syncKey = raw ? raw.toLowerCase() : null;
    if (this.syncKey) {
  ...
  async apiRequest(endpoint = '', options = {}) {
    ...
    const encodedKey = encodeURIComponent(this.syncKey);
    const fullUrl = `${this.serverUrl}/api/sync/${encodedKey}`;
  ...
  _buildSyncBody(raw) {
    ...
    for (const mod of MODULE_NAMES) {
      if (raw[mod] && raw[mod].data && (!Array.isArray(raw[mod].data) || raw[mod].data.length)) {
  ...
  _vacuumGraveyards() {
    ...
    for (const store of ['bookmarks', 'highlights', 'notes', 'plans', 'note_categories', 'plan_progress', 'reading_log']) {
  ...
  async processSync() {
    ...
    this._syncing = this._runSync(body);
