window.SearchModule = class SearchModule {
  ...
  _setupUI() {
    ...
    const panelMenu = window.PopoverService
      ? window.PopoverService.create(panel, {
          onChange: (open) => {
            discoverTab.setAttribute('aria-expanded', String(open));
            if (!open) {
    ...
    const hide = () => {
      if (panelMenu) panelMenu.hide();
      else {
        panel.classList.add('hidden');
        discoverTab.setAttribute('aria-expanded', 'false');
      }
      backdrop.classList.add('hidden');
      reset();
    };
    ...
    const show = (initialQuery = '') => {
      ...
      if (window.PopoverService) {
        if (mp) window.PopoverService.create(mp).hide();
        if (mrp) window.PopoverService.create(mrp).hide();
      } else {
      ...
      if (panelMenu) panelMenu.show();
      else panel.classList.remove('hidden');
      backdrop.classList.toggle('hidden', Boolean(panelMenu?.native));
      if (initialQuery) {
    ...
    discoverTab.addEventListener('click', (e) => {
      e.stopPropagation();
      e.preventDefault();
      if (panelMenu?.isOpen() || panel.classList.contains('open')) hide();
      else show();
    });

    if (!panelMenu?.native) backdrop.addEventListener('click', hide);

    if (!panelMenu?.native) document.addEventListener('keydown', (e) => {
    ...
    input.addEventListener('input', () => {
      ...
      this._searchTimer = setTimeout(async () => {
        if (this._requestId !== reqId) return;
        const resp = await this.search(query, { page: 1, pageSize: this._pageSize });
        if (this._requestId !== reqId) return;
        this._renderResults(resp.results, query, results, hide, resp.total);
      }, 250);
    });
  }

  open(initialQuery = '') {
    if (this._openUI) this._openUI(initialQuery);
  }

  close() {
    if (this._closeUI) this._closeUI();
  }

  _renderResults(hits, query, container, hide, total) {
    ...
    if (!hits.length && !isLoadMore) {
      ...
      if (window.UISkins) window.UISkins.decorate(container.querySelector('.search-empty'), 'search-empty');
      return;
    }
    ...
    for (const r of hits) {
      ...
      if (window.UISkins) {
        window.UISkins.decorate(btn, 'search-result');
        window.UISkins.decorate(btn.querySelector('.search-result-ref'), 'search-result-ref');
        window.UISkins.decorate(btn.querySelector('.search-result-text'), 'search-result-text');
      }
      frag.appendChild(btn);
    }
    container.appendChild(frag);
    if (window.UISkins) {
      window.UISkins.decorateTree(container, [
        ['search-results', '.search-results'],
      ]);
    }

    const canLoadMore = total != null ? (this._currentPage * this._pageSize < total) : (hits.length === this._pageSize);
    if (canLoadMore) {
      ...
      more.addEventListener('click', async () => {
        ...
        more.disabled = true;
        const resp = await this.search(query, { page: this._currentPage, pageSize: this._pageSize });
        more.remove();
        this._renderResults(resp.results, query, container, hide, resp.total);
      });
