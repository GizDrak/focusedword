window.BaseRenderer = class BaseRenderer {
  ...
  _getSettings() {
    ...
    const rs = (key) => {
      ...
      return UISkins._builtinDefaults[key];
    };
    return {
      redLetter: rs('redLetter'),
      footnotes: rs('footnotes'),
      sectionHeadings: rs('sectionHeadings'),
      poetryFormatting: rs('poetryFormatting'),
      paragraphBreaks: state.get('paragraphBreaks'),
      paragraphMode: rs('paragraphMode')
    };
  }
  ...
  renderTokenChapter(verses, bionic, strength, settings) {
    ...
    const refCls = window.UISkins ? (window.UISkins.cls('heading-crossref') || 'token-section-heading-ref') : 'token-section-heading-ref';
    frag.querySelectorAll('.' + refCls + '[data-refs]').forEach(el => {
  ...
  async applyBookmarks() {
    ...
    for (const c of document.querySelectorAll('.verse-container')) {
      ...
      if (bm) {
        ...
        c.style.borderLeft = `3px solid ${color || 'var(--accent-color)'}`;
