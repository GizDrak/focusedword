window.SwipeRenderer = class SwipeRenderer {
  ...
  _createContainer(card, bionic, strength, height, settings) {
    if (card.type === 'heading') {
      ...
      el.className = 'verse-container section-heading-container heading-card';
      ...
      const headingSkin = window.UISkins && window.UISkins.getActive();
      for (const t of card.tokens) {
        if (t.type === 'section_heading') {
          const heading = headingSkin ? window.UISkins.renderComponent('section-heading', { text: t.text || '' }) : null;
          if (heading) {
            ...
            hEl.className = 'token-section-heading';
          ...
          const headingCls = window.UISkins ? (window.UISkins.cls('section-heading') || 'sh-pill-container') : 'sh-pill-container';
          const container = el.querySelector('.' + headingCls);
          if (container) {
